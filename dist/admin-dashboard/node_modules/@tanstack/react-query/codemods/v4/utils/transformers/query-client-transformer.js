module.exports = ({
  ...
  const filterQueryClientMethodCalls = (node, methods) => utils.isIdentifier(node) && methods.includes(node.name);
  const findQueryClientMethodCalls = (importIdentifiers, methods) => {
    ...
    .filter(node => filterQueryClientMethodCalls(node.value.callee.property, methods)).filter(node => {
      ...
      return utils.isFunctionCallOf(object, utils.getSelectorByImports(importIdentifiers, 'useQueryClient'));
    });
  };
  const execute = (methods, replacer) => {
    findQueryClientMethodCalls(utils.locateImports(['QueryClient', 'useQueryClient']), methods).replaceWith(replacer);
  };
  return {
    execute
  };
