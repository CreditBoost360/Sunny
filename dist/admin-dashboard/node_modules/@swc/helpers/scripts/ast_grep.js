  ...
  const task = parseFiles([root("esm")], (err, tree) => {
    ...
    if (filename.startsWith("_ts")) {
      ...
      if (match) {
        ...
        source.update(range.start.index, range.end.index, `exports._ = exports.${alias} = require("tslib").${name};`);
        task_queue.push(fs.writeFile(root("cjs", `${filename}.cjs`), source.toString(), {
          encoding: "utf-8"
        }));
      } else {
    ...
    if (match) {
      ...
      source.update(export_start, export_end, `exports._ = exports.${func_name} = ${func_name};`);
      match.findAll({
        rule: {
          pattern: func_name,
          kind: "identifier",
      ...
      if (!export_alias) {
        task_queue.push(fs.appendFile(tree.filename(), export_shortname, "utf-8"));
      } else {
    ...
    tree.root().findAll({
      ...
      const range = match.range();
      source.update(range.start.index, range.end.index, `var ${import_binding} = require("./${import_binding}.cjs");`);
      tree.root().findAll({
        rule: {
...
function report_ts_mismatch(filename, match) {
  ...
  errors.push([`${chalk.bold.red("error")}: mismatch exported function name.`, "", `${chalk.blue("-->")} ${filename}:${match.range().start.line + 1}`, "", match.text(), chalk.red([" ".repeat(range.start.column), "^".repeat(range.end.column - range.start.column)].join("")), `${chalk.bold("note:")} The exported name should be the same as the filename.`, ""].join("\n"));
}
...
function report_export_mismatch(filename, match) {
  ...
  text.splice(offset + 1, text.length, chalk.red([" ".repeat(func_range.start.column), "^".repeat(func_range.end.column - func_range.start.column)].join("")));
  errors.push([`${chalk.bold.red("error")}: mismatch exported function name.`, "", `${chalk.blue("-->")} ${filename}:${func_range.start.line + 1}:${func_range.start.column + 1}`, "", ...text, "", `${chalk.bold("note:")} The exported name should be the same as the filename.`, ""].join("\n"));
}
...
function report_noexport(filename) {
  errors.push([`${chalk.bold.red("error")}: exported name not found`, `${chalk.blue("-->")} ${filename}`].join("\n"));
}