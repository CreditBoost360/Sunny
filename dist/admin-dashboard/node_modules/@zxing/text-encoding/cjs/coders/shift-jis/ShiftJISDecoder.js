...
var ShiftJISDecoder = /** @class */function () {
  ...
  ShiftJISDecoder.prototype.handler = function (stream, bite) {
    ...
    // return finished.
    if (bite === terminology_1.end_of_stream && this.Shift_JIS_lead === 0x00) return finished_1.finished;
    ...
    if (this.Shift_JIS_lead !== 0x00) {
      ...
      // 188 + byte − offset.
      if (utilities_1.inRange(bite, 0x40, 0x7E) || utilities_1.inRange(bite, 0x80, 0xFC)) pointer = (lead - lead_offset) * 188 + bite - offset;
      // 4. If pointer is in the range 8836 to 10715, inclusive,
      // return a code point whose value is 0xE000 − 8836 + pointer.
      if (utilities_1.inRange(pointer, 8836, 10715)) return 0xE000 - 8836 + pointer;
      // 5. Let code point be null, if pointer is null, and the
      // index code point for pointer in index jis0208 otherwise.
      var code_point = pointer === null ? null : indexes_1.indexCodePointFor(pointer, indexes_1.index('jis0208'));
    ...
    // whose value is byte.
    if (terminology_1.isASCIIByte(bite) || bite === 0x80) return bite;
    // 5. If byte is in the range 0xA1 to 0xDF, inclusive, return a
    // code point whose value is 0xFF61 − 0xA1 + byte.
    if (utilities_1.inRange(bite, 0xA1, 0xDF)) return 0xFF61 - 0xA1 + bite;
