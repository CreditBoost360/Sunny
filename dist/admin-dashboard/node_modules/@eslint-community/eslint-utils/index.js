...
function getOpeningParenOfParams(node, sourceCode) {
  return node.id ? (/** @type {Token} */
  sourceCode.getTokenAfter(node.id, isOpeningParenToken)) : (/** @type {Token} */
...
const operations = Object.freeze({
    ...
    if (args != null) {
      if (calleeNode.type === "MemberExpression") {
        ...
        if (object != null) {
          ...
          if (property != null) {
            const receiver = /** @type {Record<PropertyKey, (...args: any[]) => any>} */
            object.value;
  ...
  Literal(node) {
    const literal = /** @type {Partial<Literal> & Partial<RegExpLiteral> & Partial<BigIntLiteral>} */
    node;
  ...
  MemberExpression(node, initialScope) {
    ...
    if (object != null) {
      ...
      if (property != null) {
        if (!isGetter(/** @type {object} */object.value, /** @type {PropertyKey} */property.value)) {
          return {
            value: /** @type {Record<PropertyKey, unknown>} */object.value[(/** @type {PropertyKey} */property.value)]
          };
        }
        for (const [classFn, allowed] of getterAllowed) {
          if (object.value instanceof classFn && allowed.has(/** @type {string} */property.value)) {
            return {
              value: /** @type {Record<PropertyKey, unknown>} */object.value[(/** @type {PropertyKey} */property.value)]
            };
          }
...
function getStaticValueR(node, initialScope) {
  if (node != null && Object.hasOwnProperty.call(operations, node.type)) {
    return /** @type {VisitorCallback<any>} */operations[node.type](/** @type {TSESTreeNode} */node, initialScope);
  }
  return null;
}
...
function getStaticPropertyNameValue(node, initialScope) {
  ...
  if (nameNode.type === "Literal") {
    if (/** @type {Partial<BigIntLiteral>} */nameNode.bigint) {
      return {
        value: /** @type {BigIntLiteral} */nameNode.bigint
      };
    }
    return {
...
function getStringIfConstant(node, initialScope = null) {
  ...
  if (node && node.type === "Literal" && node.value === null) {
    const literal = /** @type {Partial<SimpleLiteral> & Partial<RegExpLiteral> & Partial<BigIntLiteral>} */
    node;
    if (literal.regex) {
...
function isParenthesized(timesOrNode, nodeOrSourceCode, optionalSourceCode) {
  /** @type {number} */
  let times, /** @type {RuleNode} */
    node, /** @type {SourceCode} */
    sourceCode, maybeLeftParen, maybeRightParen;
...
function replaceF(matcher, str, replace) {
  ...
  for (const match of matcher.execAll(str)) {
    chunks.push(str.slice(index, match.index));
    chunks.push(String(replace(...(/** @type {[string, ...string[]]} */
    /** @type {string[]} */match), match.index, match.input)));
    index = match.index + match[0].length;
  }
  chunks.push(str.slice(index));
  return chunks.join("");
...
function isHasSource(node) {
  return IMPORT_TYPE.test(node.type) && /** @type {ImportDeclaration|ExportAllDeclaration|ExportNamedDeclaration} */node.source != null;
}
const has = /** @type {<T>(traceMap: TraceMap<unknown>, v: T) => v is (string extends T ? string : T)} */
...
class ReferenceTracker {
  ...
  *iterateGlobalReferences(traceMap) {
    for (const key of Object.keys(traceMap)) {
      ...
      yield* this._iterateVariableReferences(/** @type {Variable} */variable, path, nextTraceMap, true);
    }
    for (const key of this.globalObjectNames) {
      ...
      yield* this._iterateVariableReferences(/** @type {Variable} */variable, path, traceMap, false);
    }
  }
  ...
  *iterateCjsReferences(traceMap) {
    for (const {
      ...
      yield* this._iteratePropertyReferences(/** @type {CallExpression} */node, path, nextTraceMap);
    }
  }
  ...
  *_iterateLhsReferences(patternNode, path, traceMap) {
    ...
    if (patternNode.type === "ObjectPattern") {
      for (const property of patternNode.properties) {
        ...
        yield* this._iterateLhsReferences(/** @type {AssignmentProperty} */property.value, nextPath, nextTraceMap);
      }
      return;
    }
  ...
  *_iterateImportReferences(specifierNode, path, traceMap) {
    ...
    if (type === "ImportSpecifier" || type === "ImportDefaultSpecifier") {
      ...
      yield* this._iterateVariableReferences(/** @type {Variable} */
      findVariable(this.globalScope, specifierNode.local), path, nextTraceMap, false);
      return;
    }
    if (type === "ImportNamespaceSpecifier") {
      yield* this._iterateVariableReferences(/** @type {Variable} */
      findVariable(this.globalScope, specifierNode.local), path, traceMap, false);
      return;
    }
